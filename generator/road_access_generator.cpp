                     ...
                     RoadAccessCollector::RoadAccessByVehicleType & roadAccessByVehicleType)
{
  ...
  for (uint32_t lineNo = 1;; ++lineNo)
  {
    ...
    if (!iter)
    {
      LOG(LERROR, ("Error when parsing road access: no road access type. Line:", line, "Line number:", lineNo));
      return false;
    }
    RoadAccess::Type roadAccessType;
    ...
    {
      LOG(LERROR, ("Error when parsing road access: bad pointIdx at line. Line:", line, "Line number:", lineNo));
      return false;
    }
    ++iter;
...
void RoadAccessTagProcessor::Process(OsmElement const & elem, ofstream & oss)
{
  ...
  for (size_t pointIdx = 0; pointIdx < elem.m_nds.size(); ++pointIdx)
  {
    ...
    // idx == 0 used as wildcard segment Idx, for backward compatability we store |pointIdx + 1|
    // instead of |pointIdx| for road points;
