...
public:
  ...
  constexpr RouteWeight(double weight, int32_t nonPassThroughCross, int32_t accessCross,
                        double transitTime)
    : m_weight(weight)
    , m_nonPassThroughCross(nonPassThroughCross)
    , m_accessCross(accessCross)
  ...
  int32_t GetAccessCross() const { return m_accessCross; }
  ...
  bool operator<(RouteWeight const & rhs) const
  {
    ...
    // We compare m_accessCross after m_nonPassThroughCross because we can have multiple nodes with
    // access tags on the way from the area with limited access and no access tags on the ways inside
    // this area. So we probably need to make access restriction less strict than pass through
    // restrictions e.g. allow to cross access={private, destination} and build the route with the
    // least possible number of such crosses or introduce some maximal number of
    // access={private, destination} crosses.
    if (m_accessCross != rhs.m_accessCross)
      return m_accessCross < rhs.m_accessCross;
  ...
  RouteWeight operator-() const
  {
    ...
    return RouteWeight(-m_weight, -m_nonPassThroughCross, -m_accessCross, -m_transitTime);
  }
...
private:
  // Note: consider smaller types for m_nonPassThroughCross and m_accessCross
...
constexpr RouteWeight GetAStarWeightMax<RouteWeight>()
{
  return RouteWeight(std::numeric_limits<double>::max() /* weight */,
                     ...
                     std::numeric_limits<int32_t>::max() /* accessCross */,
                     0 /* transitTime */);  // operator< preffer bigger transit time
}

template <>
constexpr RouteWeight GetAStarWeightZero<RouteWeight>()
{
  return RouteWeight(0.0 /* weight */, 0 /* nonPassThroughCross */, 0 /* accessCross */,
                     0.0 /* transitTime */);
}
